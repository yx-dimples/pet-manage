      ...
      <el-table :data="tableData" style="width: 100%" border>
        ...
        <el-table-column label="操作" width="180" align="center">
          <template v-slot="scope">
            ...
            <el-button
              size="small"
              @click="handleUpdateBeauticianInfo(scope.$index, scope.row)"
            >
              查看详情
            </el-button>
          </template>
...
<script>
import { beauticianList, deleteBeautician } from '@/api/getData'
...
export default {
  ...
  methods: {
    async getDoctorList() {
      try {
        const res = await beauticianList()
        if (res.status === 200) {
          res.data.map((item) => {
            const tableItem = {
              id: item._id,
    ...
    handleUpdateBeauticianInfo(index, row) {
      this.$router.push({
        path: '/hms/updateBeautician',
        query: { id: row.id }
      })
    },
    handleDelete(index, row) {
      ElMessageBox.alert('是否要删除该美容师信息?', '提示', {
        confirmButtonText: 'OK'
      })
        .then(async () => {
          await deleteBeautician(row.id).then((response) => {
